...
mean(y)
data_example <- list(n = n, k=length(true_coefs), X = X, y = y)

model_1 <- stan(file = '/Users/sasaatlasov/Documents/R/Logit/logit_normal.stan',
                data=data_example,
                iter=4000, 
                chains=1,
                warmup=600)
...
z <- simulation(file_path = '/Users/sasaatlasov/Documents/R/Logit/logit_normal.stan', 
                iterations = 100)
mape_results <- rbind(mape_results, z[1, ])
rmse_results <- rbind(rmse_results, z[2, ])

x <- simulation(file_path = '/Users/sasaatlasov/Documents/R/Logit/logit_logistic.stan', 
                iterations = 100)
mape_results <- rbind(mape_results, x[1, ])
rmse_results <- rbind(rmse_results, x[2, ])

c <- simulation(file_path = '/Users/sasaatlasov/Documents/R/Logit/logit_uniform.stan', 
                iterations = 100)
mape_results <- rbind(mape_results, c[1, ])
rmse_results <- rbind(rmse_results, c[2, ])

v <- simulation(file_path = '/Users/sasaatlasov/Documents/R/Logit/logit_student.stan', 
                iterations = 100)
...
write.csv(mape_results, file='/Users/sasaatlasov/Documents/R/Logit/mape.csv')
write.csv(rmse_results, file='/Users/sasaatlasov/Documents/R/Logit/rmse.csv')
